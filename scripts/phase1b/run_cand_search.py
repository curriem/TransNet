...
ims = glob.glob('/Users/mcurrie/Projects/TransiNet/data/set_%s/F*drz*' %
                set_num)
...
for filter_combo in filter_combos:
    ...
    for epoch_combo in epoch_combos:
        e1, e2 = epoch_combo
        print 'python check_for_candidates.py %s %s %s %s %s' \
                 % (set_num, filt1,filt2, str(e1),str(e2))

        print commands.getoutput('python check_for_candidates.py %s %s \
                                  %s %s %s' % (set_num, filt1, filt2, str(e1),
                                                 str(e2)))
